...
document.addEventListener("DOMContentLoaded", function () {
    ...
    button.addEventListener("click", async function () {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/programs?year=${selectedYear}`);
            ...
            data.forEach(program => {
                ...
                programElement.innerHTML = `<a href="${program.link}" target="_blank">${program.title}</a>`;
